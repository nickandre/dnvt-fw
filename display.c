...
void update_display(){
    char buf[30];
    ssd1306_clear(&disp);
    bool awaiting_computer = get_dip_value(DIP_DISABLE_LINE_SIMULATOR) == 0 && !usb_active();
    ssd1306_draw_string(&disp, 0, 0*LINE_SPACE, 1, "DNVT Switch ver 0.31");
    ssd1306_draw_string(&disp, 0, 1*LINE_SPACE, 1, "Ln  Status  Conn. To");
    if (awaiting_computer) {
        ssd1306_draw_string(&disp, 20, 3*LINE_SPACE, 1, "Awaiting");
        ssd1306_draw_string(&disp, 20, 4*LINE_SPACE, 1, "USB Host");
    } else {
        for (int i = 0; i < NUM_PHONES; i++) {
            ...
            if (phone->phone_state == plain_text || phone->phone_state == traffic_dial) {
    ...
    sprintf(buf, "Load: %02d%%|USB: %s", get_load(), configured ? usb_active() ? "activ": "inact" : "n/c");
